...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .horizon { position: absolute; left: 50%; bottom: 0; width: max(100vw, 175vh); height: auto; z-index: 1; pointer-events: none;
             transform: translate(-50%, calc(9% + var(--p) * 55%)); opacity: clamp(0, calc(1 - var(--p) * 2.4), 1); }
  ...
  .dim { opacity: .12; }
  .sun { cursor: pointer; }
  .sun-core { transform-box: fill-box; transform-origin: center; transition: transform .4s cubic-bezier(.2,.8,.2,1); }
  .sun:hover .sun-core { transform: scale(1.12); }
  .sun-lbl { font: 600 11px var(--mono); letter-spacing: .2em; fill: rgba(40,8,0,.82); pointer-events: none; }
  ...
  .legend .dot { width: 16px; height: 16px; margin-left: -4px; }
  .legend button { grid-template-columns: 16px 1fr auto !important; }
  ...
  .keys img { display: block; }
  .keys .kp { position: relative; display: block; }
  .keys .kp::after { content: ""; position: absolute; inset: -2px; border-radius: 50%; border: 1px solid #ff6b35; animation: pulse 2.6s ease-out infinite; }
  ...
  .orb { flex: none; width: 58px; height: 58px; margin: -8px -4px -8px -10px; object-fit: contain; filter: drop-shadow(0 0 18px color-mix(in srgb, var(--c) 45%, transparent)); }
  .orb.exited { width: 74px; height: 74px; margin: -16px -12px -16px -18px; }
  ...
  .chip img { width: 16px; height: 16px; margin: -3px 0 -3px -2px; object-fit: contain; }
...
<div id="stage">
  <div class="sticky">
    ...
    <img class="horizon" src="img/horizon.webp" alt="" aria-hidden="true">
    ...
    <div class="keys ui off mono">
      <span>Size = scale <img src="img/p-drinks-1.webp" width="12" height="12" alt=""><img src="img/p-drinks-1.webp" width="22" height="22" alt=""></span>
      <span>Breakout <span class="kp"><img src="img/p-protein-0.webp" width="18" height="18" alt=""></span></span>
      <span>Exited <img src="img/x-drinks.webp" width="30" height="30" alt="" style="margin:-6px -6px -6px 0"></span>
...
// Blender renders (render/render_assets.py): sprites are lit from the left; the disk fills
// 80% of a planet sprite and 50% of a ringed (exited) sprite
SEGS.forEach(g => CO.filter(c => c.galaxy === g.id).forEach((c, k) => {
  c.img = c.status === "exited" ? `img/x-${g.id}.webp` : `img/p-${g.id}-${k % 3}.webp`;
  c.k = c.status === "exited" ? 2.0 : 1.25;
}));
const segImg = g => `img/p-${g.id}-0.webp`;
...
ringOrder.forEach(g => {
  ...
  list.forEach((c, k) => {
    ...
    const face = el("g", { class: "face" }, grp);
    el("image", { href: c.img, x: -r * c.k, y: -r * c.k, width: 2 * r * c.k, height: 2 * r * c.k }, face);
    el("circle", { class: "ring-focus", r: r + 5 }, grp);
    const lbl = el("text", { class: "lbl", x: r + 7, y: 4 }, grp);
    lbl.textContent = c.name;
    planets.push({ c, g, grp, face, a0: g.phase + (k / list.length) * Math.PI * 2, delay: 0 });
  });
});
...
const sunBody = el("g", { class: "sun-core" }, sun);
const sunImg = el("image", { href: "img/sun.webp", x: -30, y: -30, width: 60, height: 60 }, sunBody);
...
function place() {
  sunImg.setAttribute("transform", `rotate(${(rot * 1.5).toFixed(2)})`);
  planets.forEach(p => {
    ...
    p.face.setAttribute("transform", `rotate(${(a * 57.2958).toFixed(1)})`);   // lit side faces the sun
...
SEGS.forEach(g => {
  ...
  legend.insertAdjacentHTML("beforeend", `<button data-seg="${g.id}" style="--c:${g.color}"><img class="dot" src="${segImg(g)}" alt=""><span class="nm">${esc(g.name)}</span><span class="ct">${n}</span></button>`);
...
const chips = list => `<div class="chips">${list.map(p => `<button type="button" class="chip" data-id="${p.id}" style="--c:${segById[p.galaxy].color}"><img src="${p.img}" alt="">${esc(p.name)}<small>${GEO_SHORT[p.geo]}</small></button>`).join("")}</div>`;
...
const VIEWS = {
  company(c) {
    ...
    return { color: g.color, title: c.name, html: `<div class="mono d-kick">${pad(g.n)} &middot; ${esc(g.name)}</div>
      <div class="d-head"><img class="orb ${c.status}" src="${c.img}" alt=""><h2 id="d-title">${esc(c.name)}</h2></div>
  ...
  segment(g) {
    return { color: g.color, title: g.name, html: `<div class="mono d-kick">Orbit ${pad(g.n)} / ${pad(SEGS.length)}</div>
      <div class="d-head"><img class="orb" src="${segImg(g)}" alt=""><h2 id="d-title">${esc(g.name)}</h2></div>
