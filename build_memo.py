...
Reads data/companies.json and emits docs/BFY-Indulgence-Memo-Arjun-Kulshrestha.pdf.
...
OUT = ROOT / "docs" / "BFY-Indulgence-Memo-Arjun-Kulshrestha.pdf"
...
A(P(f"UNITED STATES + AUSTRALIA &nbsp;&middot;&nbsp; 6 NEED-STATE GALAXIES &nbsp;&middot;&nbsp; {len(DATA['companies'])} COMPANIES", "cover_s"))
A(Spacer(1, 26 * mm))
A(P(f"ARJUN KULSHRESTHA &nbsp;&middot;&nbsp; {DATA['meta']['updated'].upper()}", "cover_s"))
A(P("Companion to the interactive map: arjunadelaide.github.io/bfy-indulgence-market-map/", "cover_s"))
A(PageBreak())
...
A(P("<b>The proof.</b> PepsiCo paid <b>$1.95B for Poppi</b> (2025) off ~$500M sales. <b>OLIPOP</b> reached <b>$1.85B, "
    ...
    "~$100M first-year revenue, raised at <b>$725M</b> in 2025, then at <b>$2.25B</b> in September 2026. This is no longer niche CPG; it's the most reliable strategic "
    "M&amp;A lane in food."))
...
A(P("I deliberately mapped need-states, not product aisles. Consumers don't shop 'low-sugar confectionery', they shop "
    ...
    "(sugar-free), Water (drinks), Flora (gut), Terra (comfort), Wind (culture &amp; kids). Each is a clickable system; "
...
meets = [
    ...
    ("Noshu (AU)", "first external raise after A$4M to A$38M in four years inside the duopoly: a rare growth-stage entry in AU BFY"),
    ...
    ("NOON (AU to US)", "national in Woolworths in January 2026 and in about 2,000 Target stores by August; I want the units per store per week at Target and what decides the next channel"),
...
A(P(f"Company database: {len(DATA['companies'])} planets", "h1"))
